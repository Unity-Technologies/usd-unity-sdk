...
namespace Unity.Formats.USD
{
    ...
    {
        MeshImportFunction<MeshSample> m_meshImporter;
        MeshImportFunction<MeshSample> m_skinnedMeshImporter;
            ...
            SceneImportOptions importOptions)
        {
            ...
            foreach (var pathAndSample in m_readMeshesJob)
            {
                ...
                try
                {
                    var go = primMap[pathAndSample.path];
    ...
    public static class MeshImporter
    {
        ...
        public static GeometrySubsets ReadGeomSubsets(Scene scene, string path)
        {
            ...
            var im = new UsdGeomImageable(prim);
            ...
            foreach (var subset in subsets)
            {
                ...
                if (!indices.Get(value, defaultTime)) continue;
            ...
            bool isDynamic)
        {
            ...
            if (smr.sharedMesh == null) smr.sharedMesh = new Mesh {name = UniqueMeshName(go.name)};
            ...
            if (isDynamic) smr.sharedMesh.MarkDynamic();
            ...
            bool isDynamic)
        {
            ...
            if (mf.sharedMesh == null) mf.sharedMesh = new Mesh {name = UniqueMeshName(go.name)};

            // We only check if a mesh is dynamic when scene.IsPopulatingAccessMask is True. It only happens when a playable is
            // created, potentially way after mesh creation.
            if (isDynamic) mf.sharedMesh.MarkDynamic();
            ...
            SceneImportOptions options)
        {
            ...
            var changeHandedness = options.changeHandedness == BasisTransformation.SlowAndSafe ||
            ...
            if (usdMesh.purpose != Purpose.Default) go.SetActive(false);
            ...
            if (options.meshOptions.topology == ImportMode.Import && usdMesh.faceVertexIndices != null)
            {
                ...
                if (options.meshOptions.triangulateMesh)
                {
                    ...
                    var offsetMapping = new int[usdMesh.faceVertexCounts.Length];
                    var curOffset = 0;
                    for (var i = 0; i < usdMesh.faceVertexCounts.Length; i++)
                    {
                    ...
                    foreach (var nameAndSubset in geomSubsets.Subsets)
                    {
                        ...
                        foreach (var faceIndex in nameAndSubset.Value)
                        {
                            ...
                            for (var i = 1; i < usdMesh.faceVertexCounts[faceIndex] - 2; i++)
                ...
                {
                    var usdIndices = newIndices ?? usdMesh.faceVertexIndices;
            ...
            if (ShouldImport(options.meshOptions.boundingBox) && hasBounds)
            {
                ...
                if (changeHandedness) usdMesh.extent.center = UnityTypeConverter.ChangeBasis(usdMesh.extent.center);
            ...
            if (usdMesh.tangents != null && ShouldImport(options.meshOptions.tangents))
            {
                ...
                if (changeHandedness)
                {
                    ...
                    for (var i = 0; i < usdMesh.tangents.Length; i++)
                    {
        ...
        public static Vector3[] UnrollPrimvars(Vector3[] vertices, int[] indices)
        {
            ...
            var newPoints = new Vector3[indicesCount];
        ...
        static bool IsFaceVaryingAttribute<T>(ImportMode mode, T[] attribute, int pointCount, int faceCount)
        {
            ...
            return ShouldImport(mode) &&
                   attribute != null &&
                   (attribute.Length > pointCount || attribute.Length == faceCount);
        }

        static bool IsFaceVaryingAttribute(ImportMode mode, PrimvarBase attribute)
        {
            return ShouldImport(mode) &&
            ...
            int[] faceVertexIndices)
        {
            ...
            for (var i = 0; i < primvars.Count; i++)
            {
                ...
                // Read the raw values.
                var val = attr.Get(0.0);
                if (val.IsEmpty()) continue;

                // TODO: We shouldn't assume it is Vector2 Array (can be Vec2, Vec3 or Vec4).
                var vec2fArray = UsdCs.VtValueToVtVec2fArray(val);
                var values = UnityTypeConverter.FromVtArray(vec2fArray);
                //
        ...
        /// </returns>
        static T[] TryGetUVSet<T>(Primvar<object> uv, bool areVerticesUnrolled, int[] faceVertexIndices)
        {
            ...
            if (uv.value.GetType() != typeof(T[])) return null;
        ...
        /// </summary>
        static void ImportUv(string path,
            Mesh unityMesh,
            int uvSetIndex,
            Primvar<object> uv,
            ...
            int[] faceVertexIndices)
        {
            ...
            if (!ShouldImport(texcoordImportMode) || uv.GetValue() == null) return;
